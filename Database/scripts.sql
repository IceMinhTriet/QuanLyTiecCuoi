...
CREATE TABLE [dbo].[hall_types]( -- loai sanh tiec
	[id] VARCHAR(12) NOT NULL PRIMARY KEY,
...
CREATE TABLE [dbo].[halls]( -- sach tiec
	[id] VARCHAR(12) NOT NULL PRIMARY KEY,
	[type] VARCHAR(12) NOT NULL REFERENCES dbo.[hall_types] ([id]),
	[name] NVARCHAR(255) NOT NULL,
    [num_of_tables] INT NOT NULL CHECK ([num_of_tables] > 0), -- capacity
...
CREATE TABLE [dbo].[reservations]( -- dat tiec
	[id] VARCHAR(12) NOT NULL PRIMARY KEY,
	[customer_id] VARCHAR(12) NOT NULL REFERENCES [dbo].[customers] (id),
	[reserving_date] SMALLDATETIME NOT NULL DEFAULT(CURRENT_TIMESTAMP),
    ...
    -- CHECK (LOWER([Buoi]) IN ('sang', 'trua', 'chieu', 'toi', N'sáng', N'trưa', N'chiều', N'tối'))
)
GO
...
CREATE TABLE dbo.[billing] (
    ...
    [total_cost] AS [hall_cost] + [service_cost] + [food_cost],  -- computed columns
...
BEGIN
    ...
    INSERT INTO dbo.billing (id, reservation_id, hall_cost, service_cost, food_cost, note, is_done)
    VALUES(@id, @reservation_id, @hall_cost, @service_cost, @food_cost, @note, @is_done)
